...
def voxelize(
    ...
    for sample in batch:
        ...
        # only voxelize spatially (not temporally as it is already discrete)
        voxel_array = np.ones(sample[0].shape[1])
        voxel_array[:3] = voxel_size
        coords = np.floor(sample[0] / voxel_array)
        ...
        if sample_coordinates.shape[1] == 4:
            temporal_stages.append(torch.from_numpy(sample_coordinates[:, -1]).long())
        else:
            temporal_stages.append(torch.zeros(sample_coordinates.shape[0], dtype=torch.long))
        ...
        if len(list_labels[0].shape) == 1:
            ...
            if mode == "test":
                    ...
                    target_full.append(
                        {
                            ...
                            "temporal_stages": torch.from_numpy(original_coordinates[i][:, -1]) if original_coordinates[i].shape[1] == 4 else torch.zeros_like(original_coordinates[i][:, -1]),
                ...
                for i in range(len(target)):
                    ...
                    target[i]["temporal_stages"] = temporal_stages[i]
...
def get_instance_masks(
    ...
    for batch_id in range(len(list_labels)):
        ...
        for instance_id in instance_ids:
            ...
            label_id = tmp[0, 0]
            #TODO: handle a variable number of change IDs (3+ sequences)
            change_id = tmp[0, 2]
...
class NoGpu:
    ...
    def __len__(self):
        """Return batch size for Lightning's automatic detection"""
        if self.inverse_maps is not None:
            return len(self.inverse_maps)  # Number of samples in batch
        elif hasattr(self.coordinates, 'shape'):
            return self.coordinates.shape[0]
        return 1
    
    @property
    def batch_size(self):
        """Explicit batch size property for Lightning"""
        return len(self)
