...
class ScannetPreprocessing(BasePreprocessing):
    def __init__(
        ...
        for mode in self.modes:
            ...
            scans_folder = scannet_special_mode
            filepaths = []
    ...
    def write_metric_spec(self):
        """Emit data/processed/{scannet|scannet200}/{name}.yaml for stmetrics.

        ScanNet has no inherent change types (static dataset). We reuse the
        RScan change taxonomy so the metric runs uniformly; every ScanNet
        instance uses change_id=0 (static).
        """
        ...
        from datasets.RScan.RScan_constants import CHANGE_LABELS, VALID_CHANGE_IDS
