...
Persist4D is a bounded persistent identity/state layer on top of local ReScene perception. Each forward produces masks, class logits, and query features; `build_local_observation` filters the current-stage candidates, and a capacity-100 `PersistentMemory` associates them to slots using feature and class similarity. The slot state contains embedding, class probability, confidence, occupancy, activity, age, last-seen stage, and a processed-stage watermark. Matched slots use confidence-scaled EMA updates; unmatched candidates create births while capacity overflow is explicitly rejected.
