...
from MinkowskiEngine.MinkowskiPooling import MinkowskiAvgPooling, MinkowskiMaxPooling, MinkowskiPoolingTranspose
import torch
...
class Res16UNetBase(ResNetBase):
    ...
    def __init__(
        ...
        self.num_levels = config.num_levels  # This is constant for Res16UNet architecture
    ...
    def network_initialization(self, in_channels, out_channels, config, D):
        ...
        ## separate setup helper pooling function 
        self._pooling = MinkowskiAvgPooling(
            kernel_size=space_n_time_m(2, 1),
            stride=space_n_time_m(2, 1),
            dimension=self.D
        )
        self._temporal_pooling = MinkowskiAvgPooling(
            kernel_size=space_n_time_m(1, 2),
            stride=space_n_time_m(1, 2),
            dimension=self.D
        )
        
        self._temporal_unpooling = MinkowskiPoolingTranspose(
            kernel_size=space_n_time_m(1, 2),
            stride=space_n_time_m(1, 2),
            dimension=self.D
        )
    ...
    def pooling(self, x):
        return self._pooling(x.float())
    
    def temporal_pool_merge(self, x):
        # first pool temporally with max pooling then unpool for and OR operation temporally
        pooled = self._temporal_pooling(x.float())
        unpooled = self._temporal_unpooling(pooled.float())
        return unpooled
        
    def sparse_from_sample(self, features, sample_sparse_tensor):
        return me.SparseTensor(
            features=features,
            coordinate_manager=sample_sparse_tensor.coordinate_manager,
            coordinate_map_key=sample_sparse_tensor.coordinate_map_key,
        )
    ...
    def forward(self, x): 
        if self.wrapper:
            ...
            # first determine positional encodings in case of early fusion
            # x and output share coordinate managers 
            with torch.no_grad():
                coordinates = me.SparseTensor(
                    features=x.raw_coordinates,
                    coordinate_manager=x_sparse.coordinate_manager,
                    coordinate_map_key=x_sparse.coordinate_map_key,
                    device=x.device,
                )
                coords = [coordinates]
                for _ in reversed(range(self.num_levels)):
                    coords.append(self._pooling(coords[-1]))
            coords.reverse()
            
            # split coords into list of coordinates per hierarchical level decomposed per batch sample
            coords = [coord.decomposed_features for coord in coords]
            
            return *self._forward(x_sparse), coords
        else:
