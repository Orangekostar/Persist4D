...
def shift_scale_points(pred_xyz, src_range, dst_range=None):
    """
    pred_xyz: B x N x d_in
    src_range: [[B x d_in], [B x d_in]] - min and max XYZ coords
    dst_range: [[B x d_in], [B x d_in]] - min and max XYZ coords
    """
    d_in = pred_xyz.shape[-1]

    if dst_range is None:
        dst_range = [
            torch.zeros(
                (src_range[0].shape[0], d_in), device=src_range[0].device
            ),
            torch.ones((src_range[0].shape[0], d_in), device=src_range[0].device),
        ]
    ...
    # Add a small epsilon to prevent division by zero if only one temporal stage
    epsilon = 1e-6
    src_diff = src_diff + epsilon
