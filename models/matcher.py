    ...
    def memory_efficient_forward(self, outputs, targets, mask_type):
        ...
        for b in range(bs):
            ...
            with autocast('cuda', enabled=False):
                ...
                # Identify queries with any non-finite values and sanitize values for loss
                bad_queries = ~torch.isfinite(out_mask).all(dim=1)
                out_mask = torch.nan_to_num(out_mask, nan=0.0, posinf=0.0, neginf=0.0)
            ...
            # Heavily penalize rows (queries) that contained non-finite values to skip them
            if bad_queries.any():
                big = torch.finfo(cost_mask.dtype).max / 4
                cost_mask[bad_queries] = big
                cost_dice[bad_queries] = big
                cost_class[bad_queries] = big
            ...
            # Validate and clean cost matrix before assignment
            num_targets = C.shape[1]
            if num_targets == 0:
                # No targets, return empty indices
                indices.append((np.array([], dtype=np.int64), np.array([], dtype=np.int64)))
                continue
            
            # Clean any remaining non-finite values
            if not torch.isfinite(C).all():
                # Replace non-finite values with a large penalty
                big_penalty = torch.finfo(C.dtype).max / 8
                C = torch.where(torch.isfinite(C), C, torch.tensor(big_penalty, dtype=C.dtype))
            
            # Convert to numpy for linear_sum_assignment
            C_np = C.numpy()
            
            # Additional validation: ensure matrix is valid
            if C_np.size == 0 or not np.all(np.isfinite(C_np)):
                # Fallback: use greedy matching or return empty
                # For safety, return empty indices if matrix is invalid
                indices.append((np.array([], dtype=np.int64), np.array([], dtype=np.int64)))
                continue
            
            # Try linear_sum_assignment with error handling
            try:
                indices.append(linear_sum_assignment(C_np))
            except ValueError as e:
                # Handle infeasible cost matrix error
                # Fallback: return empty indices (no matches)
                # This allows training to continue without crashing
                indices.append((np.array([], dtype=np.int64), np.array([], dtype=np.int64)))
