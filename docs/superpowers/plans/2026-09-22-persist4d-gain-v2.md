...
- [x] Run A-OPEN/Q-SEM/S-WORST in protocol order with explicit skips. All three L arms completed750 updates, and all prescribed CAL points cover23/23:0/250/750 for A-OPEN and Q-SEM,250/750 for S-WORST. AUX completed at2026-09-23 05:13 UTC. Q-SEM and S-WORST endpoint audits pass:750 updates,6000 local batches,24000 global draws,2 rank RNG states,1 optimizer/scheduler and scheduler epoch750; all6 Q-SEM scorer tensors remain unchanged. S-WORST's measured training cost is4.764409 GPU-hours. Watchdog fix32aa42e passed the live training-to-evaluation handoffs; its21 runner tests also pass. The A-OPEN failed attempt and all costs remain preserved; the successful AUX recovery reservation adds11.638996 GPU-hours. Keep scorer frozen and separate its steps/cost.
- [x] Complete one promoted mechanism Q-SEM-L-s45 and matched C0-L-s45 to3000, then fixed six-point CAL and one SEL evaluation each. Both FULL_ENDPOINT_AUDIT records pass:24000 local batches,96000 draws,2 rank RNG states, scheduler3000, exact recipes, finite tensors, frozen scorer unchanged. The original PERCEPTION reservation cost29.890981 GPU-hours. Resume reserialized750 checkpoint files; the two old CAL records are preserved under evaluation_invalidated_checkpoint_resave. Recomputed CAL750 covers23/23 and exactly matches both old pooled metrics and output hashes (CAL_CHECKPOINT_RESAVE_AUDIT). Both CAL winners are250. Complete SEL24/24 gives Q-SEM mean/minimum/long deltas−0.000541553/−0.011577129/−0.006101519 and C0−0.003017440/−0.009738505/−0.009431928. Both fail the adoption gate, so P=R1-D0. PERCEPTION recovery completed and resumed supervisor275387. Commit b2bf9a4 reserves only unfinished training after checking endpoint recipe/hash;22 tests pass. Numeric source remains frozen for confirmation.
- [ ] After frozen scientific execution, fix PerceptionCheckpointCallback resume handling with a regression: seed last_saved_step from restored global_step before the first accumulated batch so an existing endpoint and last.ckpt are not overwritten before an optimizer update. Preserve executed source identities in the evidence; do not silently rebind historical evaluations to changed code. Recheck this requirement before any additional resumed training.
...
- [x] CAL-lock one A*, evaluate once on SEL, keep separate from refiner candidates. CAL_LOCK selects A0-U tau0.73, with mean/minimum/long deltas−0.00166284/−0.00377811/−0.00242119 versus R1. Its single SEL evaluation completed24/24 with exact D0 parity at2026-09-23 02:06 UTC; SEL mean/minimum/long deltas are+0.00024155/−0.00037879/−0.00011113. It fails the registered adoption gate, so association retains D0. Recovery02 exited0 with COMPLETE search and selection, no failures/pruning, and a verified result hash. All association attempts and wrapper overhead total8.087655 CPU-core-hours of16. After the05:13 UTC transition, RUN_STATE records the recovered ASSOC result as COMPLETE.
- [x] Optional P repair is NOT_APPLICABLE: P remains R1, and R1 repair has no adoption signal. No extra pair is trained.
...
- [x] Write immutable FINAL_LOCK with fixed confirmation inventory before PB. Locked at2026-09-26T13:49:22.974496Z; actual file SHA e474309f47e856332b8c49cee10d050fd91520d8572fa789ba317ad968504d26. Final method R1-D0. LOCAL binds the corrected46 references and154 units per seed45/46/47; PB129/516 and ADDITIONAL111/77/32 remain fixed. Confirmation started after lock.
- [x] Seed46 component/pipeline replication is NOT_APPLICABLE because no trained new component is deployed. The pre-PB commitment contains no approved training endpoints; REPLICATE completed with no failures and no reselection.
- [ ] Complete PB 129/516 per distinct required method; LOCAL154 per prescribed seed; ADDITIONAL111/77/32 units. Preserve failed-prefix denominator and separate native FH errors.
  - PB_RECOVERY_AUDIT now verifies all three required methods complete129/516. Final R1-D0 versus repaired full FH has T2/T3/T4/T5 deltas+0.022170246/+0.000266641/−0.021255448/−0.021345839, so all-T superiority is false. ADDITIONAL native and D0 both cover111/77/32 units and40/23/8 references. No selection changes follow these results.
  - Initial LOCAL attempts failed at batch0 because strict deterministic CUDA NLL is unsupported in the diagnostic validation criterion. Commit aa451f2 wraps only no-grad criterion execution in warn-only mode, restores strict state even on exception, and leaves prediction/official metric execution strict.10 tests pass, including actual CUDA NLL failure/recovery. Original failures remain in confirmation_invalidated_local_loss. Five PB/ADDITIONAL groups are reused only after source-file hash comparison proves that LOCAL is the sole changed numerical-source file; source_reuse records retain original paths/hashes and executed identities. Current CONFIRM controller3112646/task3112650 runs LOCAL; R1 seeds45/46 have PASS154/154 across46 references. The earlier reconciliation waiter stopped on incomplete LOCAL as intended; regenerate REPORT after this recovery completes.
  - Native PB first pass produced only180/516 prefixes:336 failures reported `collator changed full-history identity`. Root cause: `_ProtocolOrderDataset.load_scan_indices` delegated scan order correctly but returned the canonical base context name. Fix64e84f7 changes only sample field3 to the requested frozen order name, preserving all other fields and source scan loading;39 evaluation tests pass, including a regression first observed failing for reverse order. Failed result is retained under confirmation_invalidated_order_identity. Independent NATIVE_PB_IDENTITY_RECOVERY runs the same locked full129/516 inventory on GPU0 with explicit fix/source hashes and reservation accounting; main CONFIRM on GPU2 continues D0. After recovery, reconcile the main confirmation summary, which may retain its earlier in-memory incomplete native row. Do not alter FINAL_LOCK or reselect from PB outcomes.
