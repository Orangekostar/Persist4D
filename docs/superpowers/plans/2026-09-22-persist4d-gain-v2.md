...
- [ ] Select at most one mechanism + same-LR C0 (or C0-only positive branch), continue fixed schedule to3000, CAL-lock then SEL-select P. FULL_PROMOTION selects exactly C0-L-s45 and Q-SEM-L-s45 by PILOT_GATE. Both completed3000; CPU FULL_ENDPOINT_AUDIT records pass on2026-09-26:24000 local batches,96000 draws,2 rank RNG states, scheduler3000, exact recipes, finite model tensors, unchanged Q-SEM scorer. The original PERCEPTION reservation cost29.890981 GPU-hours. Fixed CAL1500/2250/3000 completed23/23 for both. Selection blocked because the resumed checkpoint callback reserialized update750 on the first accumulated batch, changing file hashes; current750 progress remains6000 batches/24000 draws. Old CAL750 evidence is preserved under evaluation_invalidated_checkpoint_resave. Recovery FULL_CAL_IDENTITY_RECOVERY_INVOCATION reruns only the two stale CAL points before normal fixed CAL-lock/SEL selection. Commit b2bf9a4 prevents a retry from reserving another full training run after verifying endpoint recipe and checkpoint hash;22 tests pass. Numeric source stays frozen during evaluation. Supervisor275387 remains paused until successful PERCEPTION recovery.
