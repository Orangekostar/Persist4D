...
def _payload(*, stage_index: int = 1) -> dict[str, object]:
    history = ["scene0001_00", "scene0001_01"][: stage_index + 1]
...
def test_manifest_requires_exact_expected_unique_cache_keys(tmp_path: Path):
    ...
    manifest = build_cache_manifest([second, first], expected_keys=expected)

    assert manifest["status"] == "pass"
    assert manifest["entry_count"] == 2
    assert [entry["key"]["stage_index"] for entry in manifest["entries"]] == [0, 1]
    with pytest.raises(ValueError):
        build_cache_manifest([first], expected_keys=expected)
    with pytest.raises(ValueError):
        build_cache_manifest([first, first], expected_keys=[first["key"]])
